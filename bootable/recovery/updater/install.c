...
Value* FormatFn(const char* name, State* state, int argc, Expr* argv[]) {
    ...
    if (strcmp(partition_type, "MTD") == 0) {
    	...
    	char system_device[128];
    	transformPath("system", system_device);
    	mount(system_device, "/system", "ext4",
...
Value* WriteRawImageFn(const char* name, State* state, int argc, Expr* argv[]) {
    ...
    if(emmcEnabled) {
    	char blk_device[128];
    	transformPath(partition, blk_device);

    	FILE *dest_partition = fopen(blk_device, "wb");
