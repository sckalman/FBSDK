...
int upio_set_bluetooth_power(int on)
{
    ...
    int ret = -1;
    char buffer = '0';
    struct timespec tm_200ms = {0, 200000000};	/* 200ms - be generous */
    switch(on)
    {
    ...
    if (sz < 0) {
        ...
        ret = 0;
    ALOGE("Robin set_bluetooth_power : write(%c) OK", buffer);
    ms_delay(200);
    if (fd >= 0)
        close(fd);
...
void upio_set(uint8_t pio, uint8_t action, uint8_t polarity)
{
    ...
    switch (pio)
    {
        ...
        case UPIO_BT_WAKE:
            ...
            if (write(fd, &buffer, 1) >= 0)
            {
                ALOGD("delay 70ms for bt wakeup");
                usleep(70000);
            }
            ...
            else
            {
                ...
                ALOGD("delay 20ms for bt wakeup");
                usleep(20000);
