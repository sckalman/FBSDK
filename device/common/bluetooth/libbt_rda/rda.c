...
int set_bluetooth_power(int on)
{
    ...
    ret = ioctl(fd, IOCTL_BT_DEV_POWER, &buf);
...
int init_uart(void)
{
    ...
    if (fd < 0) 
    {
        ALOGE("Can't open serial port\n");
        return -1;
    }

    if(setup_uart_param(fd, 115200, FLOW_CTL_NONE) <0)
    {
        ALOGE("Can't set serial port\n");
        close(fd);
        return -1;
    }
    ...
    if(setup_uart_param(fd, CUST_BT_SERIAL_BOURATE, CUST_BT_SERIAL_FLOW_CTRL) <0) 
    {
        ALOGE("Can't set serial port\n");
        close(fd);
        return -1;
...
int setup_uart_param(int fd,int iBaudrate,int iFlowControl)
{
    ...
    {
        ALOGE("Can't get UART port settings\n");
        return -1;
    }
    ...
    {
        ALOGE("Can't set UART port settings\n");
        return -1;
    }
    ...
    if (tcsetattr(fd, TCSANOW, &ti) < 0) 
    {
        ALOGE("Can't set UART baud rate\n");
        return -1;
    }
...
{	   
   ...
   {   
        if (bt_vnd_cbacks)
        {
        	  //ALOGE("[###### TCC BT #######]rda_wake_chip:old(%d)new(%d).\n", chipstate, wake_assert);
